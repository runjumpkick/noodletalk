...
exports.getMessage = function(noodle, channel, req, io, userList, messageType) {
  ...
  this.shiftUserList = function() {
    userList[channel].unshift(req.session.nickname[channel]);
    if (userList[channel].length > io.sockets.clients().length) {
      userList[channel] = userList[channel].splice(io.sockets.clients().length, userList[channel].length - io.sockets.clients().length);
    }
  ...
  this.generateNickMessage = function() {
    ...
    var nickInArray = userList[channel].indexOf(nickname);
    ...
    if (nickname.toLowerCase() === oldNickname || nickInArray > -1) {
      ...
      var idx = userList[channel].indexOf(oldNickname);
      ...
      userList[channel].unshift(nickname);
    }
  ...
  if(req.body) {
    ...
    var idx = userList[channel].indexOf(req.session.nickname[channel]);
    if (idx > 0) {
      userList[channel].splice(idx, 1);
    }
    this.shiftUserList();
