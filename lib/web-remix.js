    ...
    if (protocol === 'http:' || protocol === 'https:') {
      ...
      try {
        if (media.match(serviceYoutube)) {
          ...
          result = '<iframe src="//player.vimeo.com/video/' + vimeoId + '" ' +
                   'width="' + videoWidth + '" height="' + videoHeight + '" frameborder="0" ' +
                   'webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';

        } else if (media.match(serviceImage)) {
          // If this is a non-ssl image, replace http with //
          media = media.replace('http://', '//');
          result = '<img src="' + media + '">';

        } else if (media.match(serviceAudio)) {
          var audioType = media.split('.');
          var mediaSecure = media.replace('http://', '//');
          audioType = audioType[audioType.length - 1];
          result = '<audio controls="controls" preload="none" autobuffer>' +
            '<source src="' + mediaSecure + '" type="audio/' + audioType + '" /></audio>' +
          ...
          result = '<video controls="controls" preload="none" autobuffer>' +
            '<source src="' + mediaSecure + '" ' + videoFormat + ' /></video>' +
            '<a href="' + media + '" target="_blank">' + media + '</a>';
