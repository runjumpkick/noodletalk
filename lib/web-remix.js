...
exports.generate = function(content, callback) {
  ...
  this.parseWords = function(media, callback) {
    var url = media.split('/');
  ...
  var count = 0;
  var getParsedWords = function(contentArray, self) {
    self.parseWords(contentArray[i], function(err, contentResp) {
      count ++;
      newContent += ' ' + contentResp;

      if (count === contentArray.length) {
  ...
  for (var i=0; i < contentArray.length; i++) {
    getParsedWords(contentArray, this);
  }
};
